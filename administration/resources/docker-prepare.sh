...
export $env=$version && docker compose -f $compose_file pull && \
cat $compose_file | grep 'image:' | awk '{print $2}' | sed s/\${$env}/$version/g | xargs -I{} echo {} && \
cat $compose_file | grep 'image:' | awk '{print $2}' | sed s/\${$env}/$version/g | dos2unix | xargs docker save | gzip > $output && \
